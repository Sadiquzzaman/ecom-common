...
function backend() {
  ...
  # ecom-common
  git clone -b release/staging "git@bitbucket.org:simec-web/$ecom_common.git"
  cd $ecom_common

  echo "Installing $ecom_common ......."
  cp -rf env/.env.staging env/.env
  yarn install
  ...
  cp -rf -t ../../../.. bot.sh deploy.sh deploy-aws.sh
  cd ../..
  ...
  # ecom-backend
  git clone -b release/staging "git@bitbucket.org:simec-web/$ecom_backend.git"
  cd $ecom_backend
...
function frontend() {
  ...
  # ecom-frontend
  git clone -b release/staging "git@bitbucket.org:simec-web/$ecom_frontend.git"
  cd $ecom_frontend

  echo "Installing $ecom_frontend ......."
  cp -rf src/environments/environment.staging.ts src/environments/environment.prod.ts
  yarn install
...
function solr() {
  ...
  cd $deploy_dir
  git clone -b release/staging "git@bitbucket.org:simec-web/$ecom_solr.git"
